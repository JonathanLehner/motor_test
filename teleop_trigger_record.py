...
class Frame:
    ...
    depths: list                     # uint16 depth per camera, or None if none
...
class _RealSenseSource:
    ...
    def __init__(self, serial: str | None, width: int, height: int, fps: int) -> None:
        ...
        # ponytail: depth at the same res as color; if the device doesn't offer
        # that depth mode pyrealsense raises a clear error — pick a supported one.
        cfg.enable_stream(rs.stream.depth, width, height, rs.format.z16, fps)
        profile = self._pipe.start(cfg)
        self._align = rs.align(rs.stream.color)  # depth pixels → color frame
    ...
    def read(self):
        ...
        # copy: realsense reuses the frame buffers on the next wait_for_frames().
        return (True,
                np.asanyarray(color.get_data()).copy(),
                np.asanyarray(depth.get_data()).copy())  # uint16, millimeters
...
class CameraCapture:
    ...
    def __init__(self, cam_ids: list, fps: int, width: int, height: int,
        ...
        # Which cameras deliver a depth stream (RealSense) — the writer uses this
        # to allocate 16-bit PNG folders alongside the color videos.
        self.depth_flags: list[bool] = [c.has_depth for c in self._caps]
    ...
    def _run(self) -> None:
        ...
        while self._running:
            ...
            for cap in self._caps:
                ...
                depths.append(depth)   # already copied by the source; None if no depth

            if ok:
                action = self._trigger.latest_action() if self._trigger else 0.0
                frame  = Frame(timestamp=time.time(), images=images,
                               action=action, depths=depths)
...
class _StreamingEpisode:
    ...
    def __init__(self, dataset: "LeRobotDatasetWriter", uid: int) -> None:
        ...
        # 16-bit depth PNGs stream into a per-camera folder (RealSense cams only).
        self._depth_flags = dataset.depth_flags
        for i, has_depth in enumerate(self._depth_flags):
            if has_depth:
                (self._tmp / f"cam_{i}_depth").mkdir(parents=True, exist_ok=True)
    ...
    def _consume(self) -> None:
        while True:
            ...
            if frame is None:
                return
            k = len(self._meta)
            for i, (w, img) in enumerate(zip(self._writers, frame.images)):
                w.write(img)
                if self._depth_flags[i] and frame.depths[i] is not None:
                    cv2.imwrite(
                        str(self._tmp / f"cam_{i}_depth" / f"frame_{k:06d}.png"),
                        frame.depths[i])   # uint16 → 16-bit grayscale PNG
...
class LeRobotDatasetWriter:
    ...
    def __init__(self, root: Path, cam_keys: list[str], fps: int,
                 ...
                 depth_flags: Optional[list[bool]] = None) -> None:
        self.root        = root
        ...
        # Per-camera: does this camera also record a 16-bit depth PNG stream?
        self.depth_flags = depth_flags or [False] * len(cam_keys)
    ...
    def _commit_episode(self, meta: list[tuple[float, float]], tmp_dir: Path,
        ...
        with self._lock:
            ...
            for cam_idx, cam_key in enumerate(self.cam_keys):
                ...
                # Depth PNG folder → videos/chunk/<cam_key>_depth/episode_XXXXXX/
                if self.depth_flags[cam_idx]:
                    depth_dst = (self.root / "videos" / f"chunk-{chunk:03d}"
                                 / f"{cam_key}_depth" / f"episode_{ep_idx:06d}")
                    depth_dst.parent.mkdir(parents=True, exist_ok=True)
                    (tmp_dir / f"cam_{cam_idx}_depth").replace(depth_dst)
    ...
    def _flush_meta(self) -> None:
        ...
        for cam_idx, key in enumerate(self.cam_keys):
            features[key] = {
                "dtype": "video",
            ...
            if self.depth_flags[cam_idx]:
                # Depth stored as per-frame 16-bit (uint16, millimeters) PNGs under
                # videos/<key>_depth/episode_XXXXXX/frame_NNNNNN.png (not a video).
                features[f"{key}_depth"] = {
                    "dtype": "image",
        ...
        info = {
            ...
            "depth_path": "videos/chunk-{episode_chunk:03d}/{video_key}_depth/episode_{episode_index:06d}/frame_{frame_index:06d}.png",
            "features":   features,
        }
...
def run(
    ...
    if shutil.which("ffmpeg") is None:
        raise SystemExit("ffmpeg not found on PATH — install it (macOS: "
    ...
    dataset = LeRobotDatasetWriter(output, cam_keys, fps, width, height,
                                   ...
                                   depth_flags=capture.depth_flags)
...
def main() -> None:
    ...
    # Camera
    p.add_argument("--cam-ids", type=str, nargs=2, default=None,
                   metavar=("LEFT", "RIGHT"),
                   help="Camera specs: int index (OpenCV/UVC) or 'rs[:serial]' "
                        "(Intel RealSense). Omit to auto-detect UVC capture "
                        "nodes (skips metadata nodes).")
    ...
    if args.cam_ids is None:
        found = find_capture_cameras()   # Linux/V4L2 only; empty elsewhere
        if len(found) >= 2:
            args.cam_ids = found[:2]
            print(f"[auto] detected capture cameras: {found} -> using {args.cam_ids}")
        else:
            args.cam_ids = DEFAULT_CAM_IDS
            print(f"[auto] no V4L2 auto-detect (found {found}); "
                  f"falling back to --cam-ids {DEFAULT_CAM_IDS}")
